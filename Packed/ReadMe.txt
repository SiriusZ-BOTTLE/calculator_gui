...
支持的操作符(大小写不敏感):
	...
	加号"+",减号"-",乘号"*",除号"/",幂"^",阶乘"!",取模"%"
	对数"log",正弦"sin",余弦"cos",正切"tan",
	反正弦"arcsin",反余弦"arccos",反正切"arctan",余切"cot",
	排列"ARR",组合"COM",
	左括号"(",右括号")",
	正号"+",负号"-"

	优先级请结合生活实践自行脑补,不确定优先级请使用括号.
...
闲话:
	顺带一提,我不知道为什么程序这么大...而且优化也比较令人窒息...
	我以为几百K差不多,可是居然有二十M...一定是链接了过多冗余库...
	我也不知道如何压缩,所以说,还是控制台程序好啊,就几百K,
	写完就能跑,还不用打包,资源占用也低,90年代机子都能遛.
	总之凑合用吧.

更新日志:
	------------------<版本 V0.0.1>------------------
	加入状态栏,会显示一些实用信息(有一定BUG,我也修不好)
	更新的语法检查机制(BUG云集,有问题Call我)
	修正上个版本设置菜单单词的拼写错误
	支持使用逗号分隔符分隔数字(设置菜单的最后一条现在不再是摆设)

	已知BUG:
	[1]	运算符'*'乘号的优先级高于运算符'/'除号
...
- 地址:火星殖民地/14区/32号基站/住宅2区/1单元/301室
